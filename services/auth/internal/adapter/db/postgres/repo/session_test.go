...
func TestSessionRepo_UpdateTokenHash(t *testing.T) {
	ctx := context.Background()
	userRepo := NewUserRepo(testDB.DB)
	sessionRepo := NewSessionRepo(testDB.DB)

	user := &domain.User{
		Email:        "session_updatehash@test.com",
		PasswordHash: "hashed_password",
	}
	createdUser, err := userRepo.Create(ctx, user)
	require.NoError(t, err)
	...
	err = sessionRepo.UpdateTokenHash(ctx, created.ID, newTokenHash)
	require.NoError(t, err)

	found, err := sessionRepo.GetByTokenHash(ctx, newTokenHash)
	require.NoError(t, err)
	assert.Equal(t, created.ID, found.ID)
...
func TestSessionRepo_DeleteByID(t *testing.T) {
	...
	user := &domain.User{
		Email:        "session_delete@test.com",
		PasswordHash: "hashed_password",
	}
	createdUser, err := userRepo.Create(ctx, user)
	require.NoError(t, err)

	tokenHash := "token_hash_delete"
	session := &domain.Session{
		UserID:           createdUser.ID,
		RefreshTokenHash: tokenHash,
		UserAgent:        "TestAgent/1.0",
	...
	err = sessionRepo.DeleteByID(ctx, created.ID)
	require.NoError(t, err)
